...
INSERT INTO Screens (screenName, theaterID, charge)
...
CREATE TABLE IF NOT EXISTS `Bookings` (
  `bookingsId` INT PRIMARY KEY AUTO_INCREMENT,
  `showId` INT,
  `userId` INT,
  `seats` VARCHAR(255),
  `charge` INT
...
ALTER TABLE `Shows` ADD FOREIGN KEY (`screenId`) REFERENCES `Screens` (`screeningID`);
...
ALTER TABLE `Bookings` ADD FOREIGN KEY (`showId`) REFERENCES `Shows` (`showId`);
